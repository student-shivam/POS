      ...
      <div className="dashboard-page">
        <div className="page-header">
          <div>
            ...
            <p className="page-subtitle">Real-time sales monitoring, order health, and revenue insights for your store.</p>
          </div>
        ...
        <div className="dashboard-grid">
          {[
            { label: "TODAY'S REVENUE", icon: <DollarCircleOutlined />, color: "rgba(99, 102, 241, 0.1)", textColor: "#6366f1", valueField: "todayRevenue" },
            { label: "MONTHLY REVENUE", icon: <CalendarOutlined />, color: "rgba(249, 115, 22, 0.1)", textColor: "#f97316", valueField: "monthlyRevenue" },
            { label: "TOTAL REVENUE", icon: <DollarCircleOutlined />, color: "rgba(34, 197, 94, 0.1)", textColor: "#22c55e", valueField: "totalRevenue" },
            { label: "TOTAL ORDERS", icon: <ShoppingCartOutlined />, color: "rgba(59, 130, 246, 0.1)", textColor: "#3b82f6", valueField: "totalOrders", isCount: true },
          ].map((kpi, i) => (
            <Card className="glass-morph stat-card" key={i}>
              <div className="stat-icon" style={{ background: kpi.color, color: kpi.textColor }}>{kpi.icon}</div>
              <Statistic
                title={<span style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.06em" }}>{kpi.label}</span>}
                value={kpi.isCount ? stats?.[kpi.valueField] || 0 : stats?.[kpi.valueField] || 0}
        ...
        <Row gutter={[20, 20]} style={{ marginTop: 24 }}>
          <Col xs={24} lg={16}>
            <Card
              className="chart-card glass-morph"
